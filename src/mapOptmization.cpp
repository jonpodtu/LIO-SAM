...
public:
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("lio_sam_mapOptimization", options)
    {
        ...
        auto saveMapService = [this](const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<lio_sam::srv::SaveMap::Request> req, std::shared_ptr<lio_sam::srv::SaveMap::Response> res) -> void {
            ...
            saveMapDirectory = savePCDDirectory + "/" + std::string(timestamp) + "/";
            cout << "Save directory: " << saveMapDirectory << endl;
            int unused = system((std::string("mkdir -p ") + saveMapDirectory).c_str());
            if (unused != 0) {
                cout << "Failed to create directory: " << saveMapDirectory << endl;
                res->success = false;
                return;
            }
            try {
                ...
                pcl::io::savePCDFileASCII(saveMapDirectory + "/transformations.pcd", *cloudKeyPoses6D);
                ...
                *globalMapCloud += *globalSonarCloud;
                ...
                cout << "Error saving map: " << e.what() << endl;
                res->success = false;
    ...
    void visualizeGlobalMapThread()
    {
        ...
        *globalMapCloud += *globalSonarCloud;
        ...
        // Use ls to print the files in the directory
        cout << "Files in the directory:" << endl;
        unused = system((std::string("ls ") + savePCDDirectory).c_str());
